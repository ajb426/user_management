...
async def update_user(user_id: UUID, user_update: UserUpdate, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: User = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
...
async def delete_user(user_id: UUID, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: User = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
...
async def create_user(user: UserCreate, request: Request, db: AsyncSession = Depends(get_db), email_service: EmailService = Depends(get_email_service), token: str = Depends(oauth2_scheme), current_user: User = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
    Create a new user.
...
async def update_profile(
    ...
    user = await UserService.update(db, current_user.id, update_data.model_dump(exclude_unset=True))
    if not user:
...
async def upgrade_user_to_professional(
    ...
    user_update = {
        ...
        "professional_status_updated_at": datetime.utcnow()
    }
    logger.debug(f"User update data: {user_update}") 
